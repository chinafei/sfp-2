...
def step_predict(target_date_str: str):
    print("\n" + "=" * 60)
    ...
    # 从 params.json 读取 hidden_dim（训练时写入）
    ...
    for i, seg in enumerate(segments):
        ...
        # 注意: 原 predict_only.py 中存在物理修正逻辑 (x_seq[:, 4] * 0.122)，
        #       但该列实际是通道数据而非负荷数据，系数 0.122 对真实负荷值也不适用。
        #       模型原始输出已在合理量级 (3000-4000 MW)，直接使用模型输出。
...
示例:
  ...
  conda run -n llm-on-ray python sfp-2/scripts/predict_sfp2.py --build --train --epochs 30
    ...
    # 步骤 3: 预测
    if args.date:
        ok = step_predict(args.date) and ok
    else:
